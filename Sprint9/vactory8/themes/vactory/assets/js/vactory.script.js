            ...
            var myInterval = (animated) ? setInterval(function () {
              if (intervalNumber < getMax) {
                ...
                if(isInt) { // Condition to know if it is an integer or float
                  (intervalNumber >= getMax) ? $(item).text(addCommas(getMax)) : $(item).text(Math.ceil(intervalNumber));
                }
                else {
                  (intervalNumber >= getMax) ? $(item).text(addCommas(getMax)) : $(item).text(intervalNumber.toFixed(2));
                }
      ...
      $.each(this.errorList, function (key, value) {
        ...
        if(value.method === "required") {
          if (value.element.className.includes('form-type-hidden') && value.element.name.includes('file')) {
            ...
            value.message = value.message =  Drupal.t('Field') + ' ' + _name_file + ' ' + Drupal.t('is required');
          }
...
(function ($, Drupal) {
  ...
  Drupal.vactory.utility.imgtovideo = function () {
    $(document).ready(function () {
      var $video_player = $('.play-youtube-video');
      if($video_player.length) {
        $video_player.each(function(i) {
          $(this).on('click', function (e) {
            e.preventDefault();
            var _video_parent = $(this).parents('.js-img-video').length;
            if (_video_parent) {
...
(function ($, Drupal) {
  ...
  Drupal.vactory.utility.slickSlider = function () {
    // Variables
    ...
    var _IS_MOBILE_VIEW = (matchMedia("only screen and (max-width: 768px)").matches) ? true : false;
      ...
      function initSlider() {
        $slickSlider.each(function (index, value) {
            ...
            _destroyMobile = ($(value).data('mobile-destroy') !== undefined) ? $(value).data('mobile-destroy') : false,
            $settings_slider = {
              rtl: rtlMode,
              dots: ($slider.data('dots') !== undefined) ? $slider.data('dots') : true,
              appendDots: ($slider.data('slickcontroll') !== undefined) ? $slider.parent().find('.slick-controls') : $slider,
              arrows: ($slider.data('arrows') !== undefined) ? $slider.data('arrows') : true,
              prevArrow: prevArrow,
              nextArrow: nextArrow,
              appendArrows: ($slider.data('slickcontroll') !== undefined) ? $slider.parent().find('.slick-controls') : $slider,
              ...
              autoplaySpeed: ($slider.data('autoplayspeed') !== undefined) ? $slider.data('autoplayspeed') :2000,
              ...
              fade: ($slider.data('fade') !== undefined) ? $slider.data('fade') : false,
              cssEase: ($slider.data('cssease') !== undefined) ? $slider.data('cssease') : 'cubic-bezier(0.585, -0.005, 0.635, 0.920)',
              ...
              vertical: ($slider.data('vertical') !== undefined) ? $slider.data('vertical') : false,
              verticalSwiping: ($slider.data('verticalswiping') !== undefined) ? $slider.data('verticalswiping') : false,
              easing: ($slider.data('easing') !== undefined) ? $slider.data('easing') : 'linear',
              focusOnChange: false,
              focusOnSelect: false,
              ...
              responsive: [
                {
                  breakpoint: 991.98,
                  settings: {
                    arrows: ($slider.data('tablet-arrows') !== undefined) ? $slider.data('tablet-arrows') : false,
                    dots: ($slider.data('tablet-dots')) ? $slider.data('tablet-dots') : true,
                    slidesToShow: ($slider.data('tablet-toshow') !== undefined) ? Math.floor($slider.data('tablet-toshow')) : 1,
                    slidesToScroll: ($slider.data('tablet-toscroll') !== undefined) ? Math.floor($slider.data('tablet-toscroll')) : 1,
                    centerMode: $($slider.data('tablet-centermode') !== undefined) ? $slider.data('tablet-centermode') : false,
                    centerPadding: $($slider.data('tablet-centerpadding') !== undefined) ? $slider.data('tablet-centerpadding') : "30px",
                  }
                },
                {
                  breakpoint: 575.98,
                  settings: {
                    ...
                    centerPadding: $($slider.data('mobile-centerpadding') !== undefined) ? $slider.data('mobile-centerpadding') : "30px",
                    variableWidth: $($slider.data('mobile-variablewidth') !== undefined) ? $slider.data('mobile-variablewidth') : false,
                    verticalSwiping: ($slider.data('mobile-verticalswiping') !== undefined) ? $slider.data('mobile-verticalswiping') : false,
                  }
                }
              ],
          ...
          $slider.find('img.slick-lazyload').each(function (index, value) {
            if (matchMedia('(max-width: 991.98px)').matches && $(value).data('mobile') !== undefined) {
          ...
          else if (_IS_MOBILE_VIEW && $slider.find(" > *").length > 1 && !$slider.hasClass('slick-initialized') && !_destroyMobile) {
            $slider.slick($settings_slider);
          }
      ...
      // Calculate slider indicator width
      
      function slickPagerCounter(slickSliderObject) {
        if (slickSliderObject.$dots != null) {
          var dotsWidth = parseInt(slickSliderObject.$dots.find("li").css("width"));
  
          var dotsLenght = parseFloat(100/slickSliderObject.$dots.find("li").length);
          var activeDotsIndex = parseInt(slickSliderObject.$dots.find("li.slick-active").index() + 1);
          var indicatorWidth = dotsLenght * activeDotsIndex;
  
          slickSliderObject.$dots.children(".slick-controls__active-indicator").css({
            width: dotsWidth,
            left: indicatorWidth + '%'
          });
        }
      ...
      // Fix lighthouse accessibilté bug
      // focusable element inside aria-hidden item
      $slickSlider.on('init', function(event, slick, currentSlide) {
        slick.$slideTrack.find('.slick-slide').each(function() {
          $(this).attr('aria-hidden', false)
        })
        
        // Initiat slider indicator width
        if( slick.$dots != null ) 
          slick.$dots.append('<span class="slick-controls__active-indicator"></span>');
      
        slickPagerCounter(slick);
      });
...
(function ($, Drupal, drupalSettings) {
  ...
  $(document).ready(function () {
    ...
    Drupal.vactory.utility.slickSlider();
